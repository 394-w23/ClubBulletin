...
describe("Check that all clubs are not subscribed to", () => {
    it("Joined clubs should have an unsubscribe button", () => {
        cy.visit("/manageclubs");
        cy.get("[data-cy=manageClubTab").eq(1).click();
        cy.get('[data-cy=notSubscribed]').should('contain', "You aren't subscribed to any clubs yet!");
    });
});
