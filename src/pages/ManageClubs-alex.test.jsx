...
// vi.mock("../components/ClubCard", async () => {
//   const original = await vi.importActual("../components/ClubCard");
//   return {
//     ...original,
//     toggleClubSubscription: vi.fn(),
//   };
// });
...
describe("mock user is logged in", () => {
  ...
  it("loads your feed with all your clubs", async () => {
    const { wrapper } = render(
      <ManageClubs user={userLoggedInProfile.user} data={data}></ManageClubs>,
      { wrapper: MemoryRouter }
    );    
    expect(await screen.findByText("Manage")).toBeDefined();
    expect(await screen.findByText("test club 2")).toBeDefined();
    expect(await screen.findByText("random club")).toBeDefined();
    const subscribeClubButton = screen.getAllByText("Subscribe")[0];
    fireEvent.click(subscribeClubButton);    
    // expect(screen.queryByText("random club")).toBeDefined();
    // debug();
    // expect(mockFunction).toHaveBeenCalled();
    // expect(screen.queryByText("random club")).toBeNull();
  });
});
